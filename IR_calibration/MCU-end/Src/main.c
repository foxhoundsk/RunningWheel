...
{
    if (uart.Tstate != IDLE || !escalator.intervalFlag) // start calabration after connected with PC
        return;
...
void main()
{
    ...
    IE_EA = 1;	//Enable each interrupt according to its individual mask setting.
